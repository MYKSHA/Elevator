...
| Hall, car moving | Moving | `transit_hall_pending` (if directionally allowed) |
...
1. **Assert:** latch e-stop, save FSM state and `service_dir_up/down`, force idle with doors closed, start `estop_beep` for `ESTOP_BEEP_CYCLES`.
2. **Hold** while `emergency_stop` is asserted.
3. **Deassert:** clear latch, restore saved direction and FSM state; `estop_beep` counts down independently.
...
| 1 | Idle elsewhere | `(distance << 2) + pending_count` |
